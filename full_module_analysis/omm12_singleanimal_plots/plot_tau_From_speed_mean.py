...
cond = "hab"
n_frames = 54000
sexes_to_plot = ["females"]   # oder ["males", "females"]
...
def bin_timeseries(values, bin_seconds=None):

    # kein binning → originale Zeitreihe zurückgeben
    if bin_seconds is None:
        centers = np.arange(len(values)) / (FPS * 60)
        return centers, values

    bin_size = int(bin_seconds * FPS)
...
for subgroup in subgroups:
    ...
    for grp_i, grp in enumerate(subgroup):
        ...
        for sex in sexes_to_plot:
            ...
            for mice_id in mouse_ids:
                ...
                mean_speed_mouse = np.nanmean(all_speeds, axis=0)

                group_speed_curves.append(mean_speed_mouse)

            if len(group_speed_curves) == 0:
                continue

            group_speed_curves = np.vstack(group_speed_curves)

            mean_speed_group = np.nanmean(group_speed_curves, axis=0)
            sem_speed_group = np.nanstd(group_speed_curves, axis=0) / np.sqrt(
                np.sum(~np.isnan(group_speed_curves), axis=0)
            )

            t, speed_plot = bin_timeseries(mean_speed_group, bin_seconds)
            _, sem_plot = bin_timeseries(sem_speed_group, bin_seconds)
            ...
            if len(t_fit) > 5:
                ...
                try:
                    ...
                    tau_values.append((f"{grp} {sex}", tau_hat, color))

                except RuntimeError:
                    print(f"Fit failed for {grp} {sex}")
    ...
    sex_handles = [
        mlines.Line2D([], [], color='black', linestyle=sex_layout_line[s], label=s)
        for s in sexes_to_plot
    ]
