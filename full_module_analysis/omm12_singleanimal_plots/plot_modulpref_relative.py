            ...
            for sex in ["females"]:
                ...
                for j, mice_id in enumerate(d.columns.get_level_values("mouse_ids").unique()):
                    ...
                    for i, ind in enumerate(individuals):
                        ...
                        data = (np.nancumsum(top1) - np.nancumsum(top2)) / (np.nancumsum(top1) + np.nancumsum(top2))
                        ...
                        plt.plot(time_minutes[0:27000], data[0:27000], color=color, linestyle=layout)

                        # --- Label ans Ende der Kurve ---
                        valid = np.isfinite(data)
                        if np.any(valid):
                            last_idx = np.where(valid)[0][27000]
                            x_end = time_minutes[last_idx]
                            x_end = time_minutes[27000]
                            y_end = data[last_idx]
